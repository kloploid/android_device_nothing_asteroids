...
public final class FileUtils {
    ...
    public static void writeFrameLed(int[] value) {
        int[] frame = expandZones(value);
        writeFrameLed(Arrays.toString(frame).replaceAll("\\[|\\]", "").replace(", ", " "));
    }

    /**
     * Devices whose kernel frame interface addresses physical LEDs (e.g. asteroids
     * aw20036, 36 channels) can define R.array.glyph_settings_zone_map: for each
     * physical frame position it holds the logical zone index. When the incoming
     * pattern is per-zone, expand it to the physical frame. Devices without the
     * resource (Phone 1/2) are unaffected.
     */
    private static int[] expandZones(int[] value) {
        ...
        for (int i = 0; i < zoneMap.length; i++) {
            int z = zoneMap[i];
            frame[i] = (z >= 0 && z < value.length) ? value[z] : 0;
        }
        return frame;
    }
